...
else if(inputText.toLowerCase().indexOf("battery")>-1&&inputText.toLowerCase().indexOf("what")>-1||inputText.toLowerCase().indexOf("battery")>-1&&inputText.toLowerCase().indexOf("consequence")>-1||inputText.toLowerCase().indexOf("rescue")>-1&&inputText.toLowerCase().indexOf("consent")>-1||inputText.toLowerCase().indexOf("force")>-1){document.getElementById("answer").innerHTML=`Battery is the direct application of force to another perosn (Windfield & Jolowicz on Tort, 17th Ed.2006 page 88)<br>Before rendering assistance, lifeguard must identify themselves, in life threatening emergencies, the lifeguard should assume <b>consent</b>.  For children, their consent cannot be valid, it is best to obtain consent of both parent and child. `;}
//s7-legal aspects***
...
else if(inputText.toLowerCase().indexOf("bronze medallion")>-1&&inputText.toLowerCase().indexOf("award")>-1||inputText.toLowerCase().indexOf("bronze")>-1&&inputText.toLowerCase().indexOf("medallion")>-1||inputText.toLowerCase().indexOf("bm")>-1&&inputText.toLowerCase().indexOf("award")>-1){document.getElementById("answer").innerHTML=`BRONZE MEDALLION<br>1.Theory test.<br>2.SLSS Resuscitation test.<br>3.Land Based Rescue (instructor-assessed).<br>4.Water Based Rescue (instructor-assessed)-conscious casualty 50m from safety.<br>5.Rescue fitness(3min15sec)-conscious casualty 50m from safety, non-contact clothing tow to support position.<br>6.Contact tow-conscious casualty 50m from safety.<br>7.Resuscitation rescue-unconscious submerge casualty 15m from safety(1min15sec support position)<br>8.Initiative-2 casualties 20m from safety.<br>9.swim 400m, min 3 different strokes(free, breast, survival, side recommended) `;}	
//*BM
//*POOL LIFEGUARD
